...
import type { LegalBlock, LegalPageContent, LegalSection } from "./legal-types";

function formatSectionNumber(value: number): string {
  return value.toString().padStart(2, "0");
}

function LegalBlockContent({ block }: { readonly block: LegalBlock }) {
  switch (block.type) {
    case "paragraph":
        ...
        >
          <CardContent className="space-y-2 px-4 py-4">
            ...
            <p
              className={cn(
                "text-sm leading-7",
                block.tone === "accent" ? "text-gold-100/90" : "text-gray-300"
              )}
            >
              {block.text}
            </p>
    ...
    case "labeledParagraph":
      return (
        <p className="text-sm leading-7 text-gray-300 sm:text-[0.95rem]">
          <span className="font-semibold text-white">{block.label}:</span> {block.text}
        </p>
      );
    case "externalLink":
      return (
...
function LegalSectionCard({ section }: { readonly section: LegalSection }) {
  return (
      ...
      <div className="flex items-start gap-4 sm:gap-5">
        ...
        <div className="min-w-0 flex-1 space-y-4">
          ...
          {section.blocks.map((block, blockIndex) => (
            <LegalBlockContent key={`${section.id}-${block.type}-${blockIndex}`} block={block} />
          ))}
        </div>
      </div>
    </section>
    ...
    <main className="min-h-screen bg-dark-900 pt-28 pb-20 sm:pt-32">
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 sm:px-6 lg:px-8">
        <header className="max-w-3xl space-y-5">
          ...
          <div className="space-y-4">
            <h1 className="font-serif text-4xl text-white sm:text-5xl">{content.title}</h1>
            <p className="max-w-2xl text-base leading-8 text-gray-400 sm:text-lg">{content.summary}</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-gray-400">
            ...
            <Link
              href={content.companionLink.href}
              className="font-medium text-gold-400 underline-offset-4 transition-colors hover:text-gold-300 hover:underline"
            >
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-start">
          <Card className="border border-white/8 bg-dark-800/90 py-0 shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-sm">
            ...
            <CardFooter className="flex flex-col items-start justify-between gap-4 border-t border-white/5 bg-white/3 sm:flex-row sm:items-center">
              <p className="max-w-2xl text-xs leading-6 text-gray-500">{content.footerNote}</p>
          ...
          >
            <CardHeader className="border-b border-white/5 px-5 py-5">
              ...
              <CardDescription className="text-sm leading-6 text-gray-400">
                {content.sectionsNavDescription}
              </CardDescription>
            </CardHeader>
            <CardContent className="px-5 py-5">
              <nav aria-label={content.sectionsNavLabel}>
