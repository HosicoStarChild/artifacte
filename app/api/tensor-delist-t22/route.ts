...
export async function POST(request: Request) {
  try {
    ...
    const { getDelistT22InstructionAsync, findListStatePda } = await import('@tensor-foundation/marketplace');
    const { address } = await import('@solana/kit');

    const ownerAddress = address(owner);
    const fakeSigner = { address: ownerAddress, signTransactions: async () => [] };

    const delistIx = await (getDelistT22InstructionAsync as any)({
      owner: fakeSigner,
      mint: address(mint),
    ...
    console.log(`[tensor-delist-t22] tx size: ${tx.serialize().length} bytes, mint: ${mint}`);
    ...
    console.error('[tensor-delist-t22] Error:', err);
