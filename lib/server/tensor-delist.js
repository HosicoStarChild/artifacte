...
async function buildCompressedDelistTransaction(connection, mint, owner) {
  ...
  const proofAddresses = proofFields.proof.map((proofAddress) => toPublicKey(proofAddress));
  const maybeBuildCompressedDelistTransaction = async (lookupTables, blockhashOverride) => {
    try {
      const message = new TransactionMessage({
        ...
        recentBlockhash: blockhashOverride.blockhash,
  ...
  const [programLookupTable, initialBlockhash] = await Promise.all([
  ...
  const baseLookupTables = [programLookupTable.value].filter(
    (value) => value !== null,
  );

  let txBuild = await maybeBuildCompressedDelistTransaction(baseLookupTables, initialBlockhash);
  let proofLookupTableAddress = null;

  if ((!txBuild || txBuild.txSize > MAX_SOLANA_TRANSACTION_BYTES) && proofAddresses.length > 0) {
    const authority = parseAuthorityKeypair();
    const [createLookupTableInstruction, nextProofLookupTableAddress] =
    ...
    const plannedBatchSizes = await planLookupTableSetupBatchSizes({
      ...
      fitsWithinLimit: async (batchSize, includeCreateInstruction, currentOffset) => {
        ...
        const extendLookupTableInstruction = AddressLookupTableProgram.extendLookupTable({
          ...
          authority: authority.publicKey,
          lookupTable: nextProofLookupTableAddress,
          payer: authority.publicKey,
        });
        const setupTransaction = new Transaction();

        if (includeCreateInstruction) {
          setupTransaction.add(createLookupTableInstruction);
        }

        setupTransaction.add(extendLookupTableInstruction);

        const latestBlockhash = await connection.getLatestBlockhash("confirmed");
        setupTransaction.feePayer = authority.publicKey;
        setupTransaction.recentBlockhash = latestBlockhash.blockhash;
        setupTransaction.sign(authority);

        try {
          return setupTransaction.serialize().length <= MAX_SOLANA_TRANSACTION_BYTES;
        } catch (error) {
    ...
    for (const batchSize of plannedBatchSizes) {
      ...
      setupTransaction.recentBlockhash = latestBlockhash.blockhash;
      setupTransaction.sign(authority);
      const serializedSetupTransaction = setupTransaction.serialize();

      if (serializedSetupTransaction.length > MAX_SOLANA_TRANSACTION_BYTES) {
        throw new Error("Tensor proof lookup table setup exceeds Solana size limits");
      }

      const altSignature = await connection.sendRawTransaction(serializedSetupTransaction, {
        skipPreflight: true,
      });
    ...
    proofLookupTableAddress = nextProofLookupTableAddress;

    const [proofLookupTable, finalBlockhash] = await Promise.all([
    ...
    if (!proofLookupTable.value) {
      throw new Error("Tensor proof lookup table failed to activate");
    }

    txBuild = await maybeBuildCompressedDelistTransaction(
      [...baseLookupTables, proofLookupTable.value],
      finalBlockhash,
    );
  }

  if (!txBuild || txBuild.txSize > MAX_SOLANA_TRANSACTION_BYTES) {
    throw new Error("Tensor compressed delist transaction exceeds Solana size limits");
  }

  console.log(
    `[tensor-delist] tx size: ${txBuild.txSize} bytes (proof nodes: ${proofFields.proof.length}, proof ALT: ${proofLookupTableAddress?.toBase58() ?? "none"})`,
  );

  return txBuild.txBase64;
}
